...
export default function BookingPage() {
  ...
  if (isSubmitted) {
    ...
    return (
      <div className="bg-[#F8FAFC] min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
        <div className="bg-white p-10 md:p-14 rounded-xl max-w-xl w-full text-center border border-slate-200 shadow-[0_0_30px_rgba(0,240,255,0.1)] animate-slide-in">
          ...
          <p className="text-slate-600 mb-8 text-lg font-light leading-relaxed">
            Құрметті <span className="font-semibold text-slate-900">{formData.name}</span>, сіздің жазбаңыз қабылданды. Біздің әкімші растау үшін жақын арада сізбен хабарласады.
          </p>
          <div className="bg-white/5 p-6 rounded-3xl text-left mb-10 space-y-4 border border-slate-200 shadow-sm backdrop-blur-xl">
            <p className="flex justify-between items-center"><span className="text-slate-600 font-light">Қызмет:</span> <span className="font-semibold text-slate-900 text-right">{selectedService}</span></p>
            <div className="h-px w-full bg-white/10"></div>
            <p className="flex justify-between items-center"><span className="text-slate-600 font-light">Дәрігер:</span> <span className="font-semibold text-slate-900 text-right">{selectedDoctor}</span></p>
            <div className="h-px w-full bg-white/10"></div>
            <p className="flex justify-between items-center"><span className="text-slate-600 font-light">Уақыты:</span> <span className="font-bold text-cyan-400 text-right">{formData.date}, {formData.time}</span></p>
          </div>
          <Link 
            href="/"
            className="w-full inline-flex justify-center bg-blue-600 text-slate-900 py-5 rounded-full font-bold hover:bg-blue-500 hover:scale-[1.02] transition-all duration-300 shadow-[0_0_20px_rgba(0,68,255,0.4)] hover:shadow-[0_0_25px_rgba(0,240,255,0.6)]"
          >
            Басты бетке қайту
          </Link>
        </div>
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen pt-36 pb-24 relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-4xl relative z-10">
        <div className="text-center mb-16 animate-fade-in-up">
          ...
          <h1 className="text-5xl md:text-6xl font-bold font-serif text-slate-900 mb-4 tracking-tight drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]">Қабылдауға жазылу</h1>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-[0_0_30px_rgba(0,240,255,0.05)] p-8 md:p-14 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          {/* Custom Progress Bar */}
          ...
          <form onSubmit={handleSubmit} className="relative min-h-[450px] overflow-visible">
            <AnimatePresence mode="wait" custom={direction}>
              ...
              {step === 1 && (
                ...
                >
                  <div className="text-center mb-10">
                    <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Қызмет пен дәрігер</h2>
                    <p className="text-slate-600 font-light mt-2 text-lg">Қажетті медициналық бағытты таңдаңыз</p>
                  </div>
                  
                  <div className="mb-10">
                    <label className="block text-slate-700 font-bold mb-4 text-lg">Қызмет түрі</label>
                        ...
                        <div 
                          ...
                          className={`p-6 rounded-3xl border cursor-pointer transition-all duration-300 ${
                            formData.serviceId === srv.id 
                              ...
                              : 'border-slate-200 bg-slate-50 hover:border-cyan-500/50 hover:shadow-md shadow-sky-100'
                          }`}
                        >
                          <div className={`font-bold mb-2 text-xl transition-colors ${formData.serviceId === srv.id ? 'text-cyan-400' : 'text-slate-900'}`}>{srv.title}</div>
                  ...
                  <AnimatePresence>
                    {formData.serviceId && (
                      <motion.div
                        ...
                        className="mb-4"
                      >
                        <label className="block text-slate-700 font-bold mb-4 text-lg">Дәрігер</label>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {doctors.map((doc) => (
                            <div 
                              key={doc.id}
                            ...
                            >
                              <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${formData.doctorId === doc.id ? 'bg-cyan-500 text-black shadow-[0_0_10px_rgba(0,240,255,0.4)]' : 'bg-slate-800 text-slate-600'}`}>
                                {doc.full_name.charAt(0)}
                              </div>
                              <div>
                                <div className={`font-bold text-lg transition-colors ${formData.doctorId === doc.id ? 'text-cyan-400' : 'text-slate-900'}`}>{doc.full_name}</div>
              ...
              {step === 2 && (
                ...
                >
                  <div className="text-center mb-10">
                    <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Күн мен уақыт</h2>
                    <p className="text-slate-600 font-light mt-2 text-lg">Қабылдауға ыңғайлы уақытты белгілеңіз</p>
                  </div>
                  
                  <div className="mb-10 max-w-lg mx-auto">
                    <label className="block text-slate-700 font-bold mb-4 text-lg">Күні</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-14 p-5 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-slate-50 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none text-slate-900 font-semibold text-lg transition-all shadow-sm hover:border-slate-600 cursor-pointer color-scheme-light"
                  ...
                  <AnimatePresence>
                    {formData.date && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                      >
                        <label className="block text-slate-700 font-bold mb-4 flex items-center justify-between text-lg">
                              ...
                              <motion.div 
                                ...
                                className={`p-4 text-center rounded-2xl border cursor-pointer transition-colors duration-200 font-bold text-lg flex flex-col items-center gap-2 ${
                                  formData.slotId === slot.id 
                                    ...
                                    : 'border-slate-200 bg-slate-50 hover:border-cyan-500/50 text-slate-700'
                                }`}
                              >
              ...
              {step === 3 && (
                ...
                >
                  <div className="text-center mb-10">
                    <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Байланыс мәліметтері</h2>
                    <p className="text-slate-600 font-light mt-2 text-lg">Растау үшін байланыс нөміріңізді қалдырыңыз</p>
                  </div>
                  ...
                  <div className="space-y-8 max-w-lg mx-auto">
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        className="peer w-full pt-8 pb-3 pl-14 pr-5 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-slate-50 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none text-slate-900 font-bold text-lg transition-all shadow-sm hover:border-slate-600"
                      />
                      <label 
                        htmlFor="patientName"
                    ...
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        className="peer w-full pt-8 pb-3 pl-14 pr-5 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-slate-50 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none text-slate-900 font-bold text-lg tracking-wide transition-all shadow-sm hover:border-slate-600"
                      />
                      <label 
            ...
            {/* Navigation Buttons */}
            <div className="mt-14 pt-8 border-t border-slate-200 flex gap-4 max-w-lg mx-auto">
              {step > 1 && (
                <button 
                  type="button" 
                  onClick={handlePrev}
                  className="px-6 py-4 rounded-full border border-slate-200 text-slate-600 font-bold hover:bg-slate-50 hover:text-slate-900 hover:border-slate-500 transition-all duration-300 flex items-center justify-center shrink-0 shadow-sm"
                >
                  <ChevronLeft size={24} />
                </button>
              )}
              ...
              {step < 3 ? (
                <button 
                  ...
                  className="flex-1 bg-blue-600 text-slate-900 px-8 py-4 rounded-full font-bold text-lg hover:bg-blue-500 transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(0,68,255,0.4)] flex items-center justify-center gap-2 group"
                >
